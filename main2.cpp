...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        // Напишите недостающий код
         if (count(synonyms_.at(first_word).begin(), synonyms_.at(first_word).end(), second_word)) {
        cout << "Слова синонимы: "s << first_word << ", "s << second_word << endl;
            return true;
        } else {
        cout << "Слова НЕ синонимы: "s << first_word << ", "s << second_word << endl;
        return false;}
    }
...
void TestAreSynonyms() {
    ...
    // Два слова НЕ являются синонимами друг друга
